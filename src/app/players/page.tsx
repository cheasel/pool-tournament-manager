...
export default function PlayersPage() {
  ...
  const [sl8, setSl8] = useState(4);
  const [sl9, setSl9] = useState(4);
  const [sl10, setSl10] = useState(4);
  ...
  const getSlColor = (sl: number) => {
    if (sl >= 7) return 'bg-billiard-red/10 text-billiard-red border border-billiard-red/30 shadow-[0_0_10px_rgba(239,68,68,0.15)]';
    if (sl >= 5) return 'bg-billiard-orange/10 text-billiard-orange border border-billiard-orange/30';
    if (sl >= 3) return 'bg-billiard-blue/10 text-billiard-blue border border-billiard-blue/30';
